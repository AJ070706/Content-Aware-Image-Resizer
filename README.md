...
7. Save a PNG. Highlight mode saves an original-size image with removal or insertion paths marked; Modify mode saves the resized image. **Restore original** returns the selected dimension to its starting size.
...
After an image loads, the desktop host starts **independent** width and height calculations from the original image for the selected quality mode. Changing quality or brush guidance cancels those orders and starts new ones. Each worker publishes completed seam positions in original-image coordinates. The UI fetches those positions in small batches. Highlight mode paints or erases red marks. For shrinking, Modify mode compacts the surviving pixels. For enlargement, it inserts a new pixel beside each selected seam pixel, averaging its color with an adjacent original pixel. Seams are found on a shrinking copy before insertion, so each insertion path is distinct instead of repeatedly widening the same seam. Moving the slider backward reverses the preview from the cached order. Save renders the selected seam prefix in C++, so the exported pixels match the preview.
